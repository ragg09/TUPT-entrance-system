	...
	<div class="body">
		...
		<div class="b-right">
			<div id="chooseanalytics" class="form-control">
				...
				<a href="http://localhost/tupt-entrance/html/statistic/item.php"><button>DESCRIPTIVE</button></a>
			</div>
					...
					<div class="row">
						 <div class="col-md-9" id="day" style="display: none;">
						 	<select id="select-day" style="text-align-last: center;">
								...
								<?php
									$query = "SELECT ITEM_ID, ITEM FROM items";
									$result=mysqli_query($mysqli, $query);
										while($row=mysqli_fetch_array($result))
										{
											echo '<option name="kuha-day-item" value="'.$row['ITEM_ID'].'">'.$row['ITEM'].'</option>';
						...
						<div id="chart" style="width: 60%; height: 500px; float: left; margin-left: 50px; margin-top: 50px;" class="panel-body">		
						</div>
						<div id="chart2" style="width: 34%; height: 300px; float: right" class="panel-body">		
						</div>
					</div>
				</div>
			</div>
...
<script type="text/javascript" src="../../html/item-predict.js"></script>
	...
	$(document).ready(function(){
		$("#select-day").change(function(e){//fetch data of student for logout 
				...
				$.ajax({						//INSTANTIATE AJAX
					...
					url: "item-predict-selected-day.php",				//ACTION OF THE FORM. AFTER THE ENTER KEY WAS PRESSED, THIS FILE WILL BE EXECUTED WITHOUT RELOADING.
					data: {							//DATA TO BE PASSED
						"item":$("#select-day option:selected").text(),	//VARIABLE VISITOR_ID=
		...
		$("#select-month").change(function(e){//fetch data of student for logout 
				...
				$.ajax({						//INSTANTIATE AJAX
					type: "post",					//METHOD OF THE FORM
					url: "item-predict-selected-month.php",				//ACTION OF THE FORM. AFTER THE ENTER KEY WAS PRESSED, THIS FILE WILL BE EXECUTED WITHOUT RELOADING.
					data: {							//DATA TO BE PASSED
						"item":$("#select-month option:selected").text(),	//VARIABLE VISITOR_ID=
		...
		$("#select-year").change(function(e){//fetch data of student for logout 
				...
				$.ajax({						//INSTANTIATE AJAX
					type: "post",					//METHOD OF THE FORM
					url: "item-predict-selected-year.php",				//ACTION OF THE FORM. AFTER THE ENTER KEY WAS PRESSED, THIS FILE WILL BE EXECUTED WITHOUT RELOADING.
					data: {							//DATA TO BE PASSED
						"item":$("#select-year option:selected").text(),	//VARIABLE VISITOR_ID=
