...
if (isset($_GET['year'])) {
  //laptop y setting
  for($i=1; $i<=3; $i++){
      $sql = mysqli_query($mysqli,"CALL predictbyyear({$i}, 1)");
  ...
  //toolbox y setting
  for($i=1; $i<=3; $i++){
      $sql = mysqli_query($mysqli,"CALL predictbyyear({$i}, 2)");
      $row = mysqli_fetch_row($sql);
      $toolbox_y[] = $row[0];
      mysqli_next_result($mysqli);
  }
  //projector y setting
  for($i=1; $i<=3; $i++){
      $sql = mysqli_query($mysqli,"CALL predictbyyear({$i}, 3)");
      $row = mysqli_fetch_row($sql);
      $projector_y[] = $row[0];
      mysqli_next_result($mysqli);
  }
  //drawing y setting
  for($i=1; $i<=3; $i++){
      $sql = mysqli_query($mysqli,"CALL predictbyyear({$i}, 4)");
      $row = mysqli_fetch_row($sql);
      $drawing_y[] = $row[0];
      mysqli_next_result($mysqli);
  }
  //music y setting
  for($i=1; $i<=3; $i++){
      $sql = mysqli_query($mysqli,"CALL predictbyyear({$i}, 5)");
      $row = mysqli_fetch_row($sql);
      $music_y[] = $row[0];
      mysqli_next_result($mysqli);
  }
  //sport y setting
  for($i=1; $i<=3; $i++){
      $sql = mysqli_query($mysqli,"CALL predictbyyear({$i}, 6)");
      $row = mysqli_fetch_row($sql);
      $sport_y[] = $row[0];
      mysqli_next_result($mysqli);
  }   
...
//predicting 7 years ahead based from 7 years before
for($i = 1; $i <= 3; $i++) {
...
<script>
    ...
    function drawChart() {
      ...
      var options = {
        chart: {
          title: 'PREDICTED ITEMS FOR THE NEXT 3 year',
          subtitle: 'data was based from last 3 years',
        },
        width: 900,
        height: 500,
        'backgroundColor': {
...
<script>
  ...
  google.charts.setOnLoadCallback(drawChart);
   ...
   function drawChart() {
        ...
        var options = {
          title: 'Possible Entry For Next 3 Years',
          width: '100%',
          'backgroundColor': {
            'fill': 'transparent',
         },
